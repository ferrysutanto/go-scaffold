...
func newPgModel(ctx context.Context, conf Config) (*pgModel, error) {
	...
	if conf.DB != nil {
		...
		if err := db.Ping(); err != nil {
			return nil, errors.Wrap(err, "[models][newPgModel] failed to create models pg provider. failed to ping db")
		}
	} else {
		var err error
		...
		if err != nil {
			...
			return nil, errors.Wrap(err, "[models][newPgModel] failed to create models pg provider. failed to open db")
		}
	}
